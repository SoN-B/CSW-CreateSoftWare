		...
		if (Inventory[n] == "0") { //cout <<"\n선택하신 위치에 아이템이 없습니다\n" << "------------------------\n"; 
			...
			for (int i = 0; i < 14; i++) {   //14는 items 배열 크기
				if (Inventory[n] == Items[i].Name) {  //소비아이템 사용
					...
					Cursor_Line();
					setColor(11);
					cout << "hp : " << Character->Hp - Items[i].Hp << " -> "; 
					if (Character->Hp < 0) Character->Hp = 0;
					cout << Character->Hp << "\n";
					Cursor_Line();
					cout << "mp : " << Character->Mp - Items[i].Mp << " -> ";
					if (Character->Mp < 0) Character->Mp = 0;
					cout << Character->Mp << "\n";
					Cursor_Line();
					cout << "atk : " << Character->Atk - Items[i].Atk << " -> ";
					if (Character->Atk < 0) Character->Atk = 0;
					cout << Character->Atk << "\n";
					Cursor_Line();
			...
			for (int i = 0; i < 15; i++) {   //15는 Equipments 배열 크기
				...
				if (Inventory[n] == Equipments[i].Name) {
					...
					if (Equipment_slot[Num] != "0") {//장비창 확인
						...
						Cursor_Line();
						setColor(11);
						cout << "hp : " << Character->Hp - Equipments[i].Hp << " -> ";
						if (Character->Hp < 0) Character->Hp = 0;
						cout << Character->Hp << "\n";
						Cursor_Line();
						cout << "mp : " << Character->Mp - Equipments[i].Mp << " -> ";
						if (Character->Mp < 0) Character->Mp = 0;
						cout << Character->Mp << "\n";
						Cursor_Line();
						cout << "atk : " << Character->Atk - Equipments[i].Atk << " -> ";
						if (Character->Atk < 0) Character->Atk = 0;
						cout << Character->Atk << "\n";
						Cursor_Line();
			...
			for (int i = 0; i < 5; i++) {   //5는 Gacha_Equipments 배열 크기
				...
				if (Inventory[n] == Gacha_Equipments[i].Name) {
					...
					if (Equipment_slot[Num] != "0") {//장비창 확인
						...
						Cursor_Line();
						setColor(11);
						cout << "hp : " << Character->Hp - Gacha_Equipments[i].Hp << " -> ";
						if (Character->Hp < 0) Character->Hp = 0;
						cout << Character->Hp;
						Cursor_Line();
						cout << "mp : " << Character->Mp - Gacha_Equipments[i].Mp << " -> ";
						if (Character->Mp < 0) Character->Mp = 0;
						cout << Character->Mp;
						Cursor_Line();
						cout << "atk : " << Character->Atk - Gacha_Equipments[i].Atk << " -> ";
						if (Character->Atk < 0) Character->Atk = 0;
						cout << Character->Atk;
						Cursor_Line();
